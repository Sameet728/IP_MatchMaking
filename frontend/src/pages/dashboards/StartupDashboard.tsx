...
import { MATCH_SCORES_DATA } from '../../data/mockData';
...
export const StartupDashboard: React.FC = () => {
  ...
  const { data: fetchedPatents, loading } = useFetch<any[]>('/patents?isListed=true&limit=4');
  const recommendedPatents = fetchedPatents || [];

  if (loading) return <div className="p-6 text-text-muted flex items-center gap-2"><Zap className="animate-pulse" size={16}/> Loading Dashboard...</div>;
